...
- Sustained notes and palm mutes (LP humbucker, standard tuning): **Guitar-TECHS** (Pedroza et al., ICASSP 2025), player 1 direct-input channel, CC BY 4.0. One DI note per string and fret plus palm mutes on every position, cut from the long recordings and level-matched for RiffSmith (`public/samples/gtechs/NOTICE.txt`). Notes are chosen string-aware, so a fret-5 A on the low E plays the low-E recording.
