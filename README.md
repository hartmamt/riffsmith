...
| `lib/webmcp.ts` | WebMCP bridge: the 20 tool definitions, validation, serialized queue, commit waiting, registration, console shim. |
...
- **Timeline.** The repository was scaffolded with Create Next App on 2026-08-23 and contained nothing but the scaffold. Everything else, including the entire WebMCP layer (`lib/webmcp.ts`, the 20 tools, the console shim, the bridge pattern) and the tab editor itself, was written between 2026-09-01 and the submission date, after the 2026-08-25 eligibility date. `git log` is the record.
