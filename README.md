...
- Rig side panel with persisted settings (tight, volume, mute grip, picking, double tracking, engine, cab, palm-mute bank, note bank, player rules, legato landings, ring time, feedback, NAM input, room, bass rig (capture / built-in amp / guitar rig), loop).
...
The MIT license in `LICENSE` covers RiffSmith's own code and the bundled RiffSmith Distorted, RiffSmith Clean and RiffSmith Bass captures. Bundled third-party audio and engine assets carry their own licenses, listed here.
...
Three NAM captures are bundled, all trained by Matt Hartman on his own rigs with the TONE3000 trainer and released with the repo under the MIT license: **RiffSmith Distorted** (`public/nam/models/riffsmith-distorted.nam`, the default guitar amp), **RiffSmith Clean** (`riffsmith-clean.nam`, a clean channel; set `tight` to 0 to keep the boost pedal out of it) and **RiffSmith Bass** (`riffsmith-bass.nam`, the default bass channel, running in its own NAM instance). Users can load any other `.nam` file.
