...
- Neural Amp Modeler inference in an AudioWorklet. Load a `.nam` capture from disk (every capture you load is kept in a per-browser library, and an agent can switch between them). A deployment can also ship captures: drop `.nam` files in `public/nam/models/` with a `manifest.json` (`{ "models": [{ "name", "url", "credit", "creditUrl" }] }`) and the first one becomes the default amp until the user picks something else. That directory is gitignored because most captures are not redistributable; the model runs through `@opendaw/nam-wasm` at 48 kHz, output is loudness-normalized from the model's metadata, and the synthetic cab can be added after it for amp-only captures. A1 and A2 WaveNet models are supported. The loaded model persists in IndexedDB across reloads.
- Rig side panel with persisted settings (tight, volume, mute grip, picking, double tracking, engine, cab, palm-mute bank, note bank, player rules, legato landings, NAM input, room, bass rig, loop).
