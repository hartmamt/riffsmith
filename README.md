...
- Guitar Pro import: open a .gp3/.gp4/.gp5/.gpx/.gp (or MusicXML) file, parsed in the browser by alphaTab (MPL-2.0). The first guitar track becomes the song with its meters, repeats, sections, palm mutes, hammer-ons/pull-offs, slides, bends, vibrato, dead notes, pinch harmonics and tremolo; a bass track becomes the bass lane and a percussion track the drum lane. Agents use `import_guitar_pro` with the file as base64 or with alphaTex text.
- MIDI export: a Standard MIDI file with guitar, bass and drum tracks, repeats expanded, bends and slides as pitch bend (`export midi` in the menu, `export_midi` for agents).
...
| `lib/webmcp.ts` | WebMCP bridge: the 24 tool definitions, validation, serialized queue, commit waiting, registration, console shim. |
...
- **@coderline/alphatab** (MPL-2.0) parses Guitar Pro and MusicXML files for import; it is loaded on demand and unmodified.
...
- **Timeline.** The repository was scaffolded with Create Next App on 2026-08-23 and contained nothing but the scaffold. Everything else, including the entire WebMCP layer (`lib/webmcp.ts`, the 24 tools, the console shim, the bridge pattern) and the tab editor itself, was written between 2026-09-01 and the submission date, after the 2026-08-25 eligibility date. `git log` is the record.
