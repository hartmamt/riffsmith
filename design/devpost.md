...
RiffSmith is a browser tab editor for guitar and bass, tuned for metal writing. It imports and exports ASCII tab (including messy hand-written tabs with dense chug runs, section headers, and repeat marks), supports per-bar time signatures from 2/4 to 12/8 and grids from quarters to 16th triplets, and loops any section while you keep editing. Playback runs through a lookahead AudioContext scheduler into a sampled guitar engine with persistent per-string voices: legato hammer-ons, pull-offs and taps as detune automation on one voice, real bends and releases, vibrato, slides, velocity-layered palm-mute chugs with round robins, tremolo picking as real re-picked notes with a 12 ms handover, alternate/down/up picking, and optional double tracking. The signal then goes through a built-in amp chain with a synthetic cabinet IR, or through a real Neural Amp Modeler capture running as WASM inside an AudioWorklet, loudness-normalized from the model's metadata. The sound comes from real DI recordings with open licenses (Guitar-TECHS, CC BY 4.0, an LP humbucker with palm mutes and pinch harmonics on every position; FreePats, CC0), chosen string-aware so a note plays from the string it is tabbed on, with held bends and slides crossfading into real recordings of the target pitch. An experimental physical string model (Rust compiled to WebAssembly, running in an AudioWorklet) can take over the ring-out. A rig panel holds the amp and performance settings, and users can import their own palm-mute sample banks. It opens on a Drop B arrangement of Grieg's In the Hall of the Mountain King (public domain): palm-muted theme, the answer a fifth up, a tremolo-picked reprise, and a chord finale. The pure scheduling and import layers have 29 unit tests.
